...
describe("FootballGameBet", function () {
  async function deployFixture() {
    ...
    const FootballGameBetFactory = await hre.ethers.getContractFactory("FootballGameBet");
    const stake = hre.ethers.parseEther("1");
    const startTime = (await hre.ethers.provider.getBlock("latest")).timestamp + 3600;

    const footballGameBet = await FootballGameBetFactory.connect(organiser).deploy(
      "TeamA", "TeamB", startTime, stake, organiser.address
    );
  ...
  describe("Betting", function () {
    it("Allows player to bet on home team", async function () {
      const { footballGameBet, player1, stake } = await loadFixture(deployFixture);
    ...
    it("Reverts if incorrect stake is sent", async function () {
      ...
      await expect(
        footballGameBet.connect(player1).betOnAwayTeam({ value: hre.ethers.parseEther("0.5") })
  ...
  describe("Payouts", function () {
    it("Distributes winnings correctly to home team bettors", async function () {
      const { footballGameBet, player1, player2, organiser, stake } = await loadFixture(deployFixture);

      await footballGameBet.connect(player1).betOnHomeTeam({ value: stake });
      await footballGameBet.connect(player2).betOnHomeTeam({ value: stake });

      const balanceBefore = await hre.ethers.provider.getBalance(organiser.address);

      await footballGameBet.connect(organiser).gameFinished(2, 1); // Home wins

      const balanceAfter = await hre.ethers.provider.getBalance(organiser.address);
      expect(balanceAfter).to.be.gt(balanceBefore); // Organiser got 5% fee
    });

    it("Refunds all players if game is a draw", async function () {
      const { footballGameBet, player1, player2, stake, organiser } = await loadFixture(deployFixture);
