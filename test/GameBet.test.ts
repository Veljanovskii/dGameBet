...
describe("GameBet", function () {
  ...
  describe("createFootballBet", function () {
    it("Creates a new FootballGameBet contract", async function () {
      ...
      await gameBet.connect(organiser).createFootballBet("TeamA", "TeamB", startTime, stake);
  ...
  describe("Voting", function () {
    it("Allows a valid user to vote on organiser", async function () {
      ...
      const footballGameBet = FootballGameBetFactory.attach(footballGameBetAddress);
      ...
      expect(await gameBet.canVote(organiser.address, footballGameBetAddress)).to.be.true;

      await gameBet.connect(voter).vote(organiser.address, footballGameBetAddress, 5);
    });
  });
});
