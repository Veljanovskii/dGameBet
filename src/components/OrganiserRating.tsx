...
export default function OrganiserRating({ organiser }: { organiser: `0x${string}` }) {
  ...
  const { avg, count, active } = useMemo(() => {
    ...
    const [activeFlag, totalRate, numberOfTimesRated] = data as unknown as [boolean, bigint, bigint];
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      ...
      <span className="text-gray-500">({count} {count === 1 ? 'vote' : 'votes'})</span>
      {!active && <span className="text-xs text-gray-400">• new organiser</span>}
    </div>
  );
}
