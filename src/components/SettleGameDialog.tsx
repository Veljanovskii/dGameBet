...
export default function SettleGameDialog({ betAddress }: Props) {
  ...
  const handleSettle = async () => {
    try {
      ...
      if (Number.isNaN(hg) || Number.isNaN(ag)) return alert('Enter valid integers');
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogTrigger asChild>
        <Button variant="outline" className="mt-4">Settle Game</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input type="number" value={homeGoals} onChange={(e) => setHomeGoals(e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium">Away Team Goals</label>
            <Input type="number" value={awayGoals} onChange={(e) => setAwayGoals(e.target.value)} />
          </div>
          <Button
            onClick={handleSettle}
            disabled={status === 'pending' || isMining || homeGoals === '' || awayGoals === ''}
            className="w-full"
          >
            {(status === 'pending' || isMining) ? 'Settling…' : 'Confirm and Settle'}
          </Button>
          {error && <p className="text-sm text-red-600 break-all">{String((error as any)?.message ?? error)}</p>}
        </div>
      </DialogContent>
    </Dialog>
  );
