...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const contracts = allAddresses.flatMap((address) => ([
      { address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'startTime' as const },
      { address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'getIsSettled' as const },
      { address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'homeTeam' as const },
      { address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'awayTeam' as const },
    ]));

    const basic = await client.multicall({ contracts, allowFailure: true });

    const nowMs = Date.now();
    const basicRows = allAddresses.map((address, i) => {
      ...
      const inStatus =
        status === 'upcoming' ? !hasStarted && !isSettled :
        status === 'started'  ?  hasStarted && !isSettled :
                                 isSettled;

      const matchesSearch =
        search.length >= 2
          ? homeTeam.toLowerCase().includes(search) || awayTeam.toLowerCase().includes(search)
          : true;
    ...
    const detailsContracts = pageRows.flatMap(({ address }) => ([
      { address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'organiser' as const },
      { address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'stake' as const },
      { address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'totalHomeBets' as const },
      { address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'totalAwayBets' as const },
      { address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'homeTeamPool' as const },
      { address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'awayTeamPool' as const },
      { address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'homeTeamGoals' as const },
      { address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'awayTeamGoals' as const },
    ]));

    const details = await client.multicall({ contracts: detailsContracts, allowFailure: true });

    const items = pageRows.map(({ address, startTime, isSettled, homeTeam, awayTeam }, idx) => {
      const pick = (g: number) => details[g * pageRows.length + idx];

      const organiser     = (pick(0).result ?? '0x0000000000000000000000000000000000000000') as `0x${string}`;
