...
export async function GET(req: NextRequest) {
  try {
    ...
    const contracts = [
      // 0: homeTeam
      ...addresses.map((address) => ({ address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'homeTeam' as const })),
      // 1: awayTeam
      ...addresses.map((address) => ({ address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'awayTeam' as const })),
      // 2: stake
      ...addresses.map((address) => ({ address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'stake' as const })),
      // 3: startTime
      ...addresses.map((address) => ({ address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'startTime' as const })),
      // 4: organiser
      ...addresses.map((address) => ({ address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'organiser' as const })),
      // 5: totalHomeBets
      ...addresses.map((address) => ({ address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'totalHomeBets' as const })),
      // 6: totalAwayBets
      ...addresses.map((address) => ({ address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'totalAwayBets' as const })),
      // 7: homeTeamPool
      ...addresses.map((address) => ({ address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'homeTeamPool' as const })),
      // 8: awayTeamPool
      ...addresses.map((address) => ({ address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'awayTeamPool' as const })),
      // 9: homeTeamGoals
      ...addresses.map((address) => ({ address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'homeTeamGoals' as const })),
      // 10: awayTeamGoals
      ...addresses.map((address) => ({ address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'awayTeamGoals' as const })),
      // 11: getIsSettled
      ...addresses.map((address) => ({ address, abi: FOOTBALL_GAME_BET_ABI, functionName: 'getIsSettled' as const })),
    ];
    ...
    const items = addresses.map((address, idx) => {
      ...
      return {
        ...
        organiser:    safe<`0x${string}`>(ORG, idx, '0x0000000000000000000000000000000000000000'),
